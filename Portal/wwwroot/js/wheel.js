...
function adjustColor(color, factor) {
    ...
    // Determine if input is a color name or hex
    let hex = normalizedColor;
    if (colorMap[normalizedColor]) {
        hex = colorMap[normalizedColor];
    } else if (!/^#([0-9A-Fa-f]{3}|[0-9A-Fa-f]{6})$/.test(normalizedColor)) {
        console.warn(`Invalid color format: ${color}. Defaulting to #000000.`);
        hex = '#E2E3E3'; // Fallback to black for invalid colors
    }

    // Handle 3-digit hex (e.g., #FFF) by expanding to 6-digit
    if (hex.length === 4) {
...
function GetParentDropDown() {
    $.ajax({
        url: 'Wheel/GetWheelSections', // Calls get-all action in WheelController
...
function drawWheel() {
    ...
    if (editMode) {
        for (let i = 0; i < segments; i++) {
            ...
            addClickHitbox(pencilGroup); // ✅ enlarged clickable area
            ...
            addClickHitbox(addGroup); // ✅ enlarged clickable area
            ...
            if ((i === selectedSegment || i === hoveredSegment) && wheelData[selectedSegment].Children.length <= 0) {
                ...
                addClickHitbox(childAddGroup); // ✅ enlarged clickable area
    ...
    if (isSubBoxesActive && selectedSegment !== null && wheelData[selectedSegment].Children != null) {
        ...
        if (subSegments > 0) {
            ...
            if (editMode) {
                ...
                addClickHitbox(subAddGroupBefore); // ✅ enlarged clickable area
            ...
            for (let i = 0; i < subSegments; i++) {
                ...
                if (editMode) {
                    ...
                    if ((isHoveredSub || isSelectedSub) && wheelData[selectedSegment].Children[i].Children.length <= 0) {
                        ...
                        addClickHitbox(subChildAddGroup); // ✅ enlarged clickable area
                ...
                if ((isGrandBoxesActive && i === selectedSubSegment && wheelData[selectedSegment].Children[i].Children.length > 0) ||
                    ...
                    if (editMode) {
                        ...
                        addClickHitbox(grandAddGroupBefore); // ✅ enlarged clickable area
                    ...
                    for (let j = 0; j < grandSegments; j++) {
                        ...
                        if (editMode) {
                            ...
                            addClickHitbox(grandPencilGroup); // ✅ enlarged clickable area
...
function saveEdit() {
    ...
    $.ajax({
        url: '/Wheel/EditWheelSectionFinal', // Calls Save action in WheelController
        type: 'POST',
        data: { id: parseInt(id), Name: name, Colour: colour, orderID: parseInt(order), fkParentID: parseInt(parent) },
        traditional: true,               // ensures correct binding
...
function saveDelete() {
    ...
    const WheelId = $('#editId').val();
    if (!WheelId) {
        alert("Invalid record ID.");
        return;
    }
    //  const data = { id: WheelId };
    // const Json = JSON.stringify(data);
    console.log("WheelId", WheelId);

    $.ajax({
        url: '/Wheel/DeleteWheelSection', // Calls ProcessData action in HomeController
        type: 'POST',
        data: { id: parseInt(WheelId) },   // ✅ send as form data, not JSON
        traditional: true,               // ensures correct binding
...
function addNewSection() {
    ...
    $.ajax({
        url: '/Wheel/AddWheelSection', // Calls Save action in WheelController

        type: 'POST',
        traditional: true,               // ensures correct binding
        data: { Name: name, Colour: colour, orderID: parseInt(order), fkParentID: parseInt(parent) },
        success: function (response) {
...
function AddNewSegment(item) {
    ...
    const Data = {
        ...
        fkParentID: parseInt(parentId)
    };

    $.ajax({
        url: '/wheel/SaveWheelSection', // Calls Save action in WheelController
        type: 'POST',
        data: Data,
        traditional: true,               // ensures correct binding
